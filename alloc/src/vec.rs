...
use std::{alloc::handle_alloc_error, io, mem};
...
// TODO: do i need to decorate panicking methods with `no_global_oom_handling`?
//   std decorates failable methods with `#[cfg(not(no_global_oom_handling))]`, but docs say
//   nothing about it; and if i just slap that cfg attribute onto stuff - rust start spewing
//   warnings at me.
#[cold]
fn handle_reserve_error(err: ReserveError) -> ! {
    match err {
        ReserveError::CapacityOverflow => panic!("capacity overflow"),
        ReserveError::AllocError { layout, .. } => handle_alloc_error(layout),
    }
}
...
impl<T, A: Allocator> Vec<T, A> {
    ...
    ///
    /// ```
    /// use alloc::vec::{Vec, ReserveError};
    ///
    /// let mut v = Vec::new();
    /// v.try_push(1)?;
    /// v.try_reserve(10)?;
    ...
    pub fn try_resize_with<F>(&mut self, new_len: usize, mut f: F) -> Result<(), ReserveError>
    where
        F: FnMut() -> T,
    {
        ...
        self.try_reserve(n)?;
        let spare = self.spare_capacity_mut();
        // NOTE: do not attempt to `spare.iter_mut()`.
        //   it's very-very-never-endingly slow.
        //   test_zst_reserve_errors did never finish.
        for i in 0..n {
            spare[i].write(f());
        }
        self.len += n;
...
impl<T: Clone, A: Allocator> Vec<T, A> {
    ...
    pub fn try_resize(&mut self, new_len: usize, value: T) -> Result<(), ReserveError> {
        ...
        self.try_reserve(n)?;
        let spare = self.spare_capacity_mut();
        // NOTE: do not attempt to `spare.iter_mut().take(n - 1)`.
        //   it's very-very-never-endingly slow.
        //   test_zst_reserve_errors did never finish.
        for i in 1..n {
            spare[i].write(value.clone());
        }
        // NOTE: we can write the last item directly without cloning needlessly.
        spare[0].write(value);
        self.len += n;
        Ok(())
    }
    ...
    pub fn try_from_item_in(value: T, n: usize, alloc: A) -> Result<Self, ReserveError> {
        let mut this = Self::try_with_capacity_in(n, alloc)?;
        this.try_resize(n, value)?;
        Ok(this)
    }
    ...
    pub fn try_extend_from_slice(&mut self, other: &[T]) -> Result<(), ReserveError> {
        ...
        // TODO: do i need possibly more optimal implementation?
        //
        // let n = other.len();
        // self.try_reserve(n)?;
        // let spare = self.spare_capacity_mut();
        // for i in 0..n {
        //     spare[i].write(other[i].clone());
        // }
        // self.len += n;
        // Ok(())
    }
...
__impl_eq! { [A1: Allocator, A2: Allocator] Vec<T, A1>, Vec<U, A2> }
__impl_eq! { [A: Allocator] Vec<T, A>, &[U] }
__impl_eq! { [A: Allocator] Vec<T, A>, &mut [U] }
__impl_eq! { [A: Allocator] &[T], Vec<U, A> }
__impl_eq! { [A: Allocator] &mut [T], Vec<U, A> }
__impl_eq! { [A: Allocator] Vec<T, A>, [U] }
__impl_eq! { [A: Allocator] [T], Vec<U, A> }
__impl_eq! { [A: Allocator, const N: usize] Vec<T, A>, [U; N] }
__impl_eq! { [A: Allocator, const N: usize] Vec<T, A>, &[U; N] }
...
fn test_uses_provided_allocator() {
    let temp = crate::TemporaryStorage::<1024>::new();
...
fn test_zst_reserve_errors() {
    ...
    this.try_resize_with(101, || ZST).unwrap();
    assert!(matches!(
