...
#[inline]
fn size_align_up(size: usize, align: usize) -> usize {
...
#[repr(C, align(64))]
pub struct TemporaryStorage<const SIZE: usize, const MIN_ALIGN: usize = 8> {
    data: UnsafeCell<[u8; SIZE]>,
    occupied: Cell<usize>,
    high_water_mark: Cell<usize>,
}

impl<const SIZE: usize, const MIN_ALIGN: usize> TemporaryStorage<SIZE, MIN_ALIGN> {
    ...
    pub fn allocate(&self, layout: Layout) -> *mut u8 {
        let size = size_align_up(layout.size(), layout.align().max(MIN_ALIGN));
        ...
        self.occupied.set(next_occupied);
        self.high_water_mark
            .set(self.high_water_mark.get().max(next_occupied));
    ...
    #[inline]
    pub fn set_mark(&self, mark: usize) {
        assert!(mark <= SIZE);
        self.occupied.set(mark);
    }
...
unsafe impl<const SIZE: usize> Allocator for TemporaryStorage<SIZE> {
...
fn test_temporary_storage() {
    let temp = TemporaryStorage::<1024>::new();
